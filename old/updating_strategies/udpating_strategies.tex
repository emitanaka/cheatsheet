...
\begin{center}
     \Large{\textbf{Updating Strategies Cheat Sheet}} \\
\end{center}

\section{Linear Fixed Model}
$$\bs{y} = \bs{X}\bs{\beta} + \bs{e} $$
\begin{itemize}
	\item If in the full rank model where $\bs{X}^\tp = \begin{bmatrix} \bs{X}_1^\tp & \bs{x}_n
	\end{bmatrix}$ and $\bs{x}_n^\tp$ is the $n$-th row of $\bs{X}$, then the $n$-th residual is zero as well as its variance.
\end{itemize}

\section{Linear Mixed Model}
$$\bs{y} = \bs{X}\bs{\beta} + \bs{Z}\bs{u} + \bs{e} $$
...
\nocite{Beckman1974, Cook1977}
\nocite{Christensen1992}
